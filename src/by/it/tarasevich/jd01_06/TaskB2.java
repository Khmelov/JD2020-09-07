...
public class TaskB2 {
    ...
    public static void main(String[] args) {
        ...
        replace(stringBuilder, m1);
        Pattern p2 = Pattern.compile("[-,:\\na-zA-Z]");
        Matcher m2 = p2.matcher(stringBuilder);

        //zamena ne bukvennih simvolov

        replaceNotLetterSymbal(stringBuilder, m2);
        Pattern p3 = Pattern.compile("[!?]{1,}");
        Matcher m3 = p3.matcher(stringBuilder);

        //zamena simvolov v konce predlojenia ns tothky.

        replaceEndSentence(stringBuilder, m3);
        Pattern p4 = Pattern.compile("[ ]{2,}");
        Matcher m4 = p4.matcher(stringBuilder);

//        zamenim povtori po probelam

        replaceSeveralSpaces(stringBuilder, m4);
        String varString = stringBuilder.toString();

//        sozdadim massiv strok

        String[] arraysString;
        arraysString = varString.trim().split("[\\.]");

//        obregem probeli


        trimSpaces(arraysString);

//        sort massiva po vozrostaniiy


        arraySort(arraysString);
        printArray(arraysString);
    ...
    private static void replaceSeveralSpaces(StringBuilder stringBuilder, Matcher m4) {
        int pos = 0;
        while (m4.find(pos)) {
            stringBuilder.replace(m4.start(), m4.end(), " ");
            pos = m4.start() + 1;
        }
    }
